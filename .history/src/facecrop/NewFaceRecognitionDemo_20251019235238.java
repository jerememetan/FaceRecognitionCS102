...
import java.nio.ByteBuffer;
                    if (!shouldProcess) {
                        AppLogger.info("[Reject] Face skipped: frame not processed (frame skip logic)");
                        continue;
                    }
                    ...
                    try {
                        ...
                        ImageProcessor.ImageQualityResult qualityResult = imageProcessor.validateImageQualityDetailed(faceColor);
                        ...
                        for (int p = 0; p < personEmbeddings.size(); p++) {
                            ...
                            if (centroid != null) {
                                ...
                                if (centroidScore < CENTROID_PREFILTER_THRESHOLD) {
                                    personScores.add(0.0);
                                    prefilterSkipped++;
                                    AppLogger.info("[Reject] Face rejected: Centroid prefilter threshold not met.");
                                    continue;
                                }
                            }
                        ...
                        if (personScores.isEmpty()) {
                            ...
                            if (accept) {
                                ...
                                AppLogger.info("[Reject] Face rejected: Recognition thresholds not met.");
                            }
            ...
            while (running && capture.read(webcamFrame)) {
                ...
                for (Rect rect : detectedFaces) {
                    ...
                    try {
                        ...
                        // 2. Pose/alignment check (require both eyes detected and roughly horizontal)
                        boolean poseOk = false;
                        try {
                            org.opencv.objdetect.CascadeClassifier eyeDetector = new org.opencv.objdetect.CascadeClassifier(
                                    "data/resources/haarcascade_eye.xml");
                            if (!eyeDetector.empty()) {
                                Mat gray = new Mat();
                                if (faceColor.channels() > 1) {
                                    Imgproc.cvtColor(faceColor, gray, Imgproc.COLOR_BGR2GRAY);
                                } else {
                                    gray = faceColor.clone();
                                }
                                MatOfRect eyes = new MatOfRect();
                                eyeDetector.detectMultiScale(gray, eyes, 1.1, 3, 0, new Size(20, 20), new Size());
                                org.opencv.core.Rect[] eyeArray = eyes.toArray();
                                if (eyeArray.length >= 2) {
                                    Arrays.sort(eyeArray, java.util.Comparator.comparingInt(eyeRect -> eyeRect.x));
                                    org.opencv.core.Point leftCenter = new org.opencv.core.Point(
                                            eyeArray[0].x + eyeArray[0].width / 2.0,
                                            eyeArray[0].y + eyeArray[0].height / 2.0);
                                    org.opencv.core.Point rightCenter = new org.opencv.core.Point(
                                            eyeArray[eyeArray.length - 1].x + eyeArray[eyeArray.length - 1].width / 2.0,
                                            eyeArray[eyeArray.length - 1].y
                                                    + eyeArray[eyeArray.length - 1].height / 2.0);
                                    double dy = Math.abs(leftCenter.y - rightCenter.y);
                                    double dx = Math.abs(leftCenter.x - rightCenter.x);
                                    // Accept only if eyes are roughly horizontal and not too close together
                                    if (dx > 30 && dy < 20) {
                                        poseOk = true;
                                    }
                                }
                            ...
                            // If pose check fails, default to not accepting
                        }
                        if (!poseOk) {
                            lastDisplayText = "unknown";
                            lastDisplayColor = new Scalar(0, 0, 255);
                        ...
                        if (personScores.isEmpty()) {
                            ...
                            if (accept && best < 0.75) {
                                accept = false;
                                System.out.println("[Decision] REJECT: Below minimum confidence threshold (0.75)");
                            }
    ...
    private double computeFusedScore(byte[] queryEmb, List<byte[]> person, double[] centroid) {
        ...
        // Use top-3 for scoring (helps dampen occasional noisy exemplars)
        ...
        double exemplarScore = 0.65 * maxTopK + 0.35 * avgTopK;
