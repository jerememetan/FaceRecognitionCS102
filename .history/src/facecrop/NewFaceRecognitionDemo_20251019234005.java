...
public class NewFaceRecognitionDemo extends JFrame implements IConfigChangeListener {
                        ...
                        try {
                            ...
                            if (!eyeDetector.empty()) {
                                ...
                                if (eyeArray.length >= 2) {
                                    Arrays.sort(eyeArray, java.util.Comparator.comparingInt(rect -> rect.x));
    ...
    private double cosineSimilarity(byte[] queryEmb, double[] centroid) {
        ...
        // âœ… CRITICAL: Normalize q after decoding!
        // OpenFace outputs L2-normalized embeddings, but decoding loses this.
        // Centroid is already normalized in computeCentroid().
