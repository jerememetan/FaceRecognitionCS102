...
public class NewFaceRecognitionDemo extends JFrame implements IConfigChangeListener {
    ...
    private void initializeOpenCV() {
        ...
        if (!capture.isOpened()) {
            AppLogger.error("Error opening webcam with all backends and indices!");
            AppLogger.error("Troubleshooting tips:");
            AppLogger.error("1. Make sure no other applications are using the camera");
            AppLogger.error("2. Check camera permissions in Windows Settings > Privacy & Security > Camera");
            AppLogger.error("3. Try restarting your computer");
            AppLogger.error("4. Update your camera drivers");
            AppLogger.error("5. Try a different USB port if using external camera");
            throw new RuntimeException("Camera Initialization failed. See logs for troubleshooting tips.");
        }
    ...
    private VideoCapture openCameraWithFallback() {
        int[] cameraIndices = {AppConfig.getInstance().getCameraIndex(), 0, 1, 2, -1}; // Try configured index first, then 0,1,2, and -1 for any available
        int[][] backends = {
            {org.opencv.videoio.Videoio.CAP_ANY}, // Try default first
            {org.opencv.videoio.Videoio.CAP_DSHOW},
            {org.opencv.videoio.Videoio.CAP_MSMF},
            {org.opencv.videoio.Videoio.CAP_WINRT} // Try WinRT as last resort
        };

        for (int cameraIndex : cameraIndices) {
            if (cameraIndex == -1) {
                // Special case: try to find any available camera
                AppLogger.info("Trying to find any available camera...");
                for (int testIndex = 0; testIndex < 10; testIndex++) {
                    VideoCapture testCap = new VideoCapture(testIndex);
                    if (testCap.isOpened()) {
                        AppLogger.info("Found available camera at index " + testIndex);
                        testCap.release();
                        VideoCapture cap = new VideoCapture(testIndex);
                        if (cap.isOpened()) {
                            AppLogger.info("Camera opened successfully at index " + testIndex + " with default backend");
                            return cap;
                        }
                    }
                    testCap.release();
            ...
            for (int[] backend : backends) {
                AppLogger.info("Attempting to open camera " + cameraIndex + " with backend " + backend[0]);

                VideoCapture cap = null;
                try {
                    if (backend[0] == org.opencv.videoio.Videoio.CAP_ANY) {
                        cap = new VideoCapture(cameraIndex);
                    } else {
                        cap = new VideoCapture(cameraIndex, backend[0]);
                    }
                    ...
                    if (cap.isOpened()) {
                        ...
                        if (cap.read(testFrame)) {
                            testFrame.release();
                            AppLogger.info("Camera opened successfully at index " + cameraIndex + " with backend " + backend[0]);
                            return cap;
                        } else {
                            AppLogger.warn("Camera opened but cannot read frames at index " + cameraIndex + " with backend " + backend[0]);
                        }
                    } else {
                        AppLogger.warn("Failed to open camera at index " + cameraIndex + " with backend " + backend[0]);
                    }
                } catch (Exception e) {
                    AppLogger.warn("Exception opening camera at index " + cameraIndex + " with backend " + backend[0] + ": " + e.getMessage());
                } finally {
        ...
        AppLogger.error("All camera backends and indices failed!");
    ...
    private void configureCamera(VideoCapture cap) {
        try {
            ...
            // Try MJPEG codec, but don't fail if it doesn't work
            ...
            AppLogger.info(String.format("Camera configured: %dx%d @ %.1f fps (MJPEG: %s)",
                    (int)actualWidth, (int)actualHeight, actualFps, codecSet ? "yes" : "no"));

            if (!widthSet || !heightSet) {
                AppLogger.warn("Some camera properties could not be set - using camera defaults");
            }
