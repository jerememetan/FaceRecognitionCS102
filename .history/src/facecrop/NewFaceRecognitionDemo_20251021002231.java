        ...
        for (int cameraIndex : cameraIndices) {
            for (int i = 0; i < backends.length; i++) {
                ...
                if (cap.isOpened()) {
                    ...
                    if (canRead) {
                        AppLogger.info("Camera opened successfully: index " + cameraIndex + " with " + backendName + " backend");
                        return cap;
                    } else {
                        AppLogger.warn("Camera opened but cannot read frames: index " + cameraIndex + " with " + backendName);
                        cap.release();
                    }
                } else {
