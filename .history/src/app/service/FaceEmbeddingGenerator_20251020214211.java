...
/**
 * FaceEmbeddingGenerator - TRAINING/ENROLLMENT ONLY
 * 
 * This class is responsible for generating face embeddings during the enrollment phase.
 * It processes captured training images and stores embeddings to disk.
 * 
 * For LIVE RECOGNITION preprocessing, use LiveRecognitionPreprocessor instead.
 * 
 * CRITICAL: This class expects BGR color images (400x400) from training captures.
 * NO grayscale conversion is performed to preserve color features for OpenFace.
 */
public class FaceEmbeddingGenerator {
     ...
     * This is called during TRAINING/ENROLLMENT phase.
     * 
     * @param faceImage Preprocessed BGR image (should be 400x400 from training captures)
     * @return Embedding byte array
     */
    ...
    /**
     * CRITICAL FIX: Removed GRAY→BGR conversion.
     * Expects BGR input, preserves color throughout.
     */
    private byte[] generateDeepEmbedding(Mat faceImage) {
        try {
            // ✅ FIXED: Accept color image directly, no grayscale conversion
            if (faceImage.channels() != 3) {
                System.err.println("❌ WARNING: Expected 3-channel BGR image, got " + faceImage.channels() + " channels");
                // Emergency fallback: convert to BGR if needed
            ...
            // Convert to float [0,1] and create blob
            processedImage.convertTo(processedImage, CvType.CV_32F, 1.0 / 255.0);
            
            // swapRB=true converts BGR→RGB for OpenFace model
    ...
    /**
     * Fallback feature-based embedding for when OpenFace model is unavailable.
     */
    ...
    /**
     * Process captured training images and generate embeddings.
     * This is called during enrollment phase ONLY.
     */
    public BatchProcessingResult processCapturedImages(
        ...
        for (String imagePath : imagePaths) {
            try {
                ...
                // ✅ TRAINING PREPROCESSING: Minimal processing preserving color
                // Training images are already 400x400 BGR from FaceDetection
    ...
    /**
     * ✅ TRAINING PREPROCESSING: Minimal processing for 400x400 BGR training images
     * NO grayscale conversion - preserves color for OpenFace
     */
    private Mat preprocessForTraining(Mat faceImage) {
        try {
            // Apply light denoising (training images are high quality)
