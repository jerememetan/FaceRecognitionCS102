...
import app.service.FaceAligner;
...
public class FaceEmbeddingGenerator {
    ...
    public byte[] generateEmbedding(Mat faceImage) {
        return generateEmbedding(faceImage, null);
    }
    ...
    private byte[] generateDeepEmbedding(Mat faceImage, Rect faceRect) {
        try {
            ...
            // *** CRITICAL FIX: Add face alignment (only if faceRect is provided) ***
            Mat aligned;
            if (faceRect != null) {
                ...
                // No face rect provided, skip alignment and resize directly
                aligned = new Mat();
                Imgproc.resize(processed, aligned, INPUT_SIZE, 0, 0, Imgproc.INTER_CUBIC);
            }
            processed.release();
