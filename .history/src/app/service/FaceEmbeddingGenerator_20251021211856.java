...
public class FaceEmbeddingGenerator {
    ...
    private byte[] generateDeepEmbedding(Mat faceImage) {
        try {
            ...
            if (faceImage.channels() != 3) {
                System.err
                        .println("❌ WARNING: Expected 3-channel BGR image, got " + faceImage.channels() + " channels");
    ...
    public BatchProcessingResult processCapturedImages(
        ...
        if (generatedEmbeddings.size() >= 5) {
            ...
            if (generatedEmbeddings.size() >= 3) { // Need at least 3 for meaningful quality analysis
                weakRemovedCount = detectAndRemoveWeakEmbeddings(generatedEmbeddings, embeddingPaths, successfulImagePaths);
        ...
        try {
            ...
            for (int i = 0; i < avgSimilarities.length; i++) {
                ...
                if (isWeakByDeviation || isWeakByAbsolute) {
                    weakIndices.add(i);
                    System.out.println(String.format("  Weak embedding detected: index=%d, avgSim=%.4f, threshold=%.4f, absThresh=%.4f",
    ...
    public static class BatchProcessingResult {
        ...
        public BatchProcessingResult(int processedCount, int removedOutlierCount, int removedWeakCount, String message) {
