...
public class FaceDetection {
    ...
    public FaceEmbeddingGenerator.BatchProcessingResult processCapturedImages(
        ...
        if (generatedEmbeddings.size() >= 5) {
            ...
            removedCount = embeddingGenerator.detectAndRemoveOutliers(generatedEmbeddings, embeddingPaths,
                    successfulImagePaths);
            weakRemovedCount = embeddingGenerator.detectAndRemoveWeakEmbeddings(generatedEmbeddings, embeddingPaths,
                    successfulImagePaths);
            if (removedCount > 0 || weakRemovedCount > 0) {
                System.out.println(
                        "✓ Auto-removed " + removedCount + " outlier and " + weakRemovedCount + " weak embedding(s)");
                if (progressCallback != null) {
                    progressCallback.onProgress("Removed " + removedCount + " outlier and " + weakRemovedCount
                            + " weak image(s) automatically");
                }
            }
        }
