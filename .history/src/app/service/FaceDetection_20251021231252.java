    ...
    public FaceCaptureResult captureAndStoreFaceImages(Student student, int numberOfImages,
        ...
        while (capturedCount < numberOfImages && attemptCount < maxAttempts) {
            ...
            if (detectionResult.hasValidFace()) {
                ...
                if (faceROI != null) {
                    // Validate image quality (sharpness, brightness, contrast) - more lenient for face capture
        ...
        if (capturedCount > 0) {
            ...
            // Detect faces in saved images for better alignment
            java.util.List<Rect> detectedFaceRects = new java.util.ArrayList<>();
            for (String imagePath : capturedImages) {
                try {
                    Mat image = Imgcodecs.imread(imagePath);
                    if (!image.empty()) {
                        Rect faceRect = detectFaceInImage(image);
                        detectedFaceRects.add(faceRect);
                        image.release();
                    } else {
                        detectedFaceRects.add(null);
                    }
                } catch (Exception e) {
                    System.err.println("Failed to detect face in saved image: " + imagePath);
                    detectedFaceRects.add(null);
                }
            }
            ...
            batchResult = embeddingGenerator
                    .processCapturedImages(capturedImages, detectedFaceRects, imageProcessor, progressCallback);
