...
public class FaceEmbeddingGenerator {
    ...
    private void initializeFaceDetector() {
        try {
            ...
            if (new java.io.File(modelConfiguration).exists() && new java.io.File(modelWeights).exists()) {
                dnnFaceDetector = Dnn.readNetFromTensorflow(modelWeights, modelConfiguration);
                System.out.println("✅ DNN face detector loaded for embedding processing");
            } else {
                System.err.println("⚠️ DNN model files not found; face detection unavailable for embedding processing");
                dnnFaceDetector = null;
            }
        } catch (Exception e) {
            System.err.println("❌ Failed to load DNN face detector: " + e.getMessage());
            dnnFaceDetector = null;
        }
    }
    ...
    public BatchProcessingResult processCapturedImages(
        ...
        for (String imagePath : imagePaths) {
            try {
                ...
                // Detect face in the saved image to get faceRect for alignment
                Rect faceRect = detectFaceInImage(image);

                if (faceRect != null) {
                    // Extract face ROI using detected rectangle
                    Mat faceROI = new Mat(image, faceRect);

                    // Generate embedding with both face ROI and face rectangle
    ...
    private int detectAndRemoveWeakEmbeddings(java.util.List<byte[]> embeddings,
        ...
        try {
            ...
            // Define weak embedding criteria:
            // - Score below mean - 0.5 * stdDev (more lenient than outliers)
            // - Absolute score below 0.85 (good quality threshold)
            double weakThreshold = Math.max(0.75, mean - 0.5 * stdDev);
            double absoluteWeakThreshold = 0.85;
    ...
    // Helper method to detect face in saved image for embedding processing
