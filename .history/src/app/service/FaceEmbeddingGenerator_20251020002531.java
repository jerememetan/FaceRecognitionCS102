...
public class FaceEmbeddingGenerator {
    ...
    public Mat preprocessFaceForLiveRecognition(Mat faceROI) {
        ...
        try {
            ...
            // ✅ CRITICAL: Upscale to 400x400 first to match training pipeline
            // Training: 400x400 captured → denoise → resize 96x96
            // Live: Variable-size ROI → upscale 400x400 → denoise → resize 96x96
            Mat upscaled = new Mat();
            Imgproc.resize(color, upscaled, new Size(400, 400), 0, 0, Imgproc.INTER_CUBIC);
            color.release();

            Mat processed = preprocessFaceForRecognition(upscaled, null);
            upscaled.release();
            return processed;
