...
public class FaceDetection {
    ...
    public FaceCaptureResult captureAndStoreFaceImages(Student student, int numberOfImages,
        ...
        if (capturedCount > 0) {
            ...
            batchResult = processCapturedImages(capturedImages, imageProcessor, progressCallback);
    ...
    public FaceEmbeddingGenerator.BatchProcessingResult processCapturedImages(
        ...
        for (String imagePath : imagePaths) {
            try {
                ...
                // Need to detect face to get faceRect for alignment
                Rect faceRect = detectFaceInImage(image);

                if (faceRect != null) {
                    Mat faceROI = new Mat(image, faceRect);
                    byte[] embedding = embeddingGenerator.generateEmbedding(faceROI, faceRect);
                    ...
                    System.err.println("❌ No face detected in image: " + imagePath);
                }
        ...
        if (generatedEmbeddings.size() >= 5) {
            ...
            removedCount = embeddingGenerator.detectAndRemoveOutliers(generatedEmbeddings, embeddingPaths, successfulImagePaths);
            weakRemovedCount = embeddingGenerator.detectAndRemoveWeakEmbeddings(generatedEmbeddings, embeddingPaths, successfulImagePaths);
            if (removedCount > 0 || weakRemovedCount > 0) {
                System.out.println("✓ Auto-removed " + removedCount + " outlier and " + weakRemovedCount + " weak embedding(s)");
                if (progressCallback != null) {
                    progressCallback.onProgress("Removed " + removedCount + " outlier and " + weakRemovedCount + " weak image(s) automatically");
                }
            }
        }

        int totalRemoved = removedCount + weakRemovedCount;
        return new FaceEmbeddingGenerator.BatchProcessingResult(processedCount, removedCount, weakRemovedCount,
                "Processed " + processedCount + " embeddings, removed " + totalRemoved + " low-quality");
    }

    // Helper method to detect face in saved image
    private Rect detectFaceInImage(Mat image) {
        ...
        try {
            ...
            System.err.println("Face detection failed in image: " + e.getMessage());
            return null;
        }
    }
