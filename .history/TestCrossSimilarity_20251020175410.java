...
public class TestCrossSimilarity {
    ...
    public static double[] decodeEmbedding(byte[] emb) {
        if (emb == null || emb.length != 128 * 4) return null;
        ...
        // L2 normalize
        double norm = 0.0;
        for (double x : dv) norm += x * x;
        norm = Math.sqrt(Math.max(norm, 1e-12));
        for (int i = 0; i < dv.length; i++) dv[i] /= norm;
        return dv;
    }
    ...
    public static void main(String[] args) throws Exception {
                ...
                for (double[] a : embA) {
                    for (double[] b : embB) {
                        ...
                        if (i != j) { // Only cross-student pairs
                            if (sim > maxSim) { maxSim = sim; maxPair = students.get(i) + " vs " + students.get(j); }
                            if (sim < minSim) { minSim = sim; minPair = students.get(i) + " vs " + students.get(j); }
                        }
                        if (sim > localMax) localMax = sim;
                        if (sim < localMin) localMin = sim;
                    }
                }
        ...
        System.out.print("          ");
        for (String s : students) System.out.printf("%16s", s);
        System.out.println();
        for (int i = 0; i < students.size(); i++) {
